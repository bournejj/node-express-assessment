...
describe("POST /auth/login", function() {
  ...
  test("should return 401 if wrong password or username is passed in", async function() {
    const response = await request(app)
      .post("/auth/login")
      .send({
        username: "u4",
...
describe("GET /users", function() {
  ...
  // Bug 1 - should return only username, first_name and last_name but returns all infromation
  test("should list all users and return {username, first_name, last_name}", async function() {
    const response = await request(app)
      .get("/users")
      .send({ _token: tokens.u1 });
    expect(response.statusCode).toBe(200);
...
describe("GET /users/[username]", function() {
  ...
  // Test Bug 2 - Should return 404 if user not found
  test("should return 404 if no user is found", async function() {
    const response = await request(app)
      .get("/users/u4")
      .send({ _token: tokens.u1 });
...
describe("DELETE /users/[username]", function() {
  ...
  // Bug 3 - Should return 404 if user not found
